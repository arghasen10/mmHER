...
confidenceMetricBreath = np.zeros(shape=(MAX_NUM_PEAKS_SPECTRUM), dtype=float)         # Confidence Metric associated with each Breathing Spectrum Peak
...
confidenceMetricHeart = np.zeros(shape=(MAX_NUM_PEAKS_SPECTRUM))          # Confidence Metric associated with each Cardiac Spectrum Peak
...
MEDIAN_WINDOW_LENGTH = 20               # Median window length (for the heart-rate picks up the median of the last MEDIAN_WINDOW_LENGTH measurements)
pBufferHeartRate = np.zeros(shape=(MEDIAN_WINDOW_LENGTH), dtype=float)       # Maintains a history of the Previous "MEDIAN_WINDOW_LENGTH" heart rate measurements
pBufferBreathingRate = np.zeros(shape=(MEDIAN_WINDOW_LENGTH), dtype=float)   # Maintains a history of the Previous "MEDIAN_WINDOW_LENGTH" heart rate measurements
pBufferHeartRate_4Hz = np.zeros(shape=(MEDIAN_WINDOW_LENGTH))
alpha_breathing = 0.1
alpha_heart = 0.05
prevRangeBins = np.zeros(shape=(256), dtype=np.complex128)
first_time = 1
...
rangeEnd_meter = 1.1
rangeBinStartIndex = math.floor(rangeStart_meter/cfg.RANGE_RESOLUTION) # Range-bin index corresponding to the Starting range in meters
rangeBinEndIndex   = math.floor(rangeEnd_meter/cfg.RANGE_RESOLUTION)      # Range-bin index corresponding to the Ending range in meters
...
def clutter_removal_C(rangeResult):
    ...
    alphaClutter = 0.01
    prevRangeBins = alphaClutter*rangeResult + (1-alphaClutter)*prevRangeBins
    rangeBinMax = np.argmax(np.absolute(prevRangeBins- rangeResult))
    return rangeBinMax

    
    for i in range(rangeResult.shape[0]):
        
        prevRangeResult = alphaClutter * rangeResult[i] + (1 - alphaClutter) * prevRangeBins[i]
...
def computeEnergyHarmonics(pAbsSpectrum, spectrumStartIndex, spectrumEndIndex, freqWindowSize):
    ...
    for index in range(spectrumStartIndex, spectrumEndIndex):
        ...
        for indexInnerLoop in range(freqWindowSize):
            sum_value += pAbsSpectrum[window1StartIndex + indexInnerLoop] + pAbsSpectrum[window2StartIndex + indexInnerLoop]
...
def process_indiv_vitalsigns(range_indv_bin):
    ...
    outputFilterBreathOut, pDelayBreath = filter_IIR_BiquadCascade(phaseUsedComputation, pFilterCoefsBreath, pScaleValsBreath, pDelayBreath, IIR_FILTER_BREATH_NUM_STAGES)
    outputFilterHeartOut, pDelayHeart  = filter_IIR_BiquadCascade(phaseUsedComputation, pFilterCoefsHeart_4Hz, pScaleValsHeart_4Hz, pDelayHeart, IIR_FILTER_HEART_NUM_STAGES)
    ...
    numPeaksHeart = find_Peaks(pVitalSigns_Heart_CircularBuffer, pPeakLocsHeart, pPeakValues, 0, circularBufferSizeHeart - 1)
    if numPeaksHeart != 0:
    ...
    if guiFlag_GainControl == 1:
        computeAGC(pVitalSigns_Heart_CircularBuffer, circularBufferSizeHeart, motionDetection_BlockSize, motionDetection_Thresh)
    ...
    if PERFORM_XCORR:
        computeAutoCorrelation (pVitalSigns_Heart_CircularBuffer,  circularBufferSizeHeart , pXcorr, xCorr_minLag, xCorr_maxLag)
    ...
    if FLAG_APPLY_WINDOW:
        ...
        for index_win in range(DOPPLER_WINDOW_SIZE):
            ...
            pVitalSignsBuffer_Cplx[index_win].real    = scale_heartWfm * tempFloat * pVitalSigns_Heart_CircularBuffer[index_win]
            pVitalSignsBuffer_Cplx[index_WinEnd].real = scale_heartWfm * tempFloat * pVitalSigns_Heart_CircularBuffer[index_WinEnd]
        ...
        for loopIndexBuffer in range(DOPPLER_WINDOW_SIZE, circularBufferSizeHeart - DOPPLER_WINDOW_SIZE):
            pVitalSignsBuffer_Cplx[loopIndexBuffer].real = scale_heartWfm*pVitalSigns_Heart_CircularBuffer[loopIndexBuffer]
    ...
    pVitalSignsSpectrumTwiddle32x32, pVitalSignsBuffer_Cplx,pVitalSigns_SpectrumCplx = DSP_fft32x32(pVitalSignsSpectrumTwiddle32x32, heartWfm_Spectrum_FftSize, pVitalSignsBuffer_Cplx,pVitalSigns_SpectrumCplx)

    pVitalSigns_Heart_AbsSpectrum = MmwDemo_magnitudeSquared(pVitalSigns_SpectrumCplx,pVitalSigns_Heart_AbsSpectrum, heartWfm_Spectrum_FftSize)

    # Pick the Peaks in the Breathing Spectrum
    numPeaks_BreathSpectrum = find_Peaks(pVitalSigns_Breath_AbsSpectrum, pPeakIndex, pPeakValues, breath_startFreq_Index, breath_endFreq_Index)
    indexNumPeaks = numPeaks_BreathSpectrum if (numPeaks_BreathSpectrum < MAX_NUM_PEAKS_SPECTRUM) else MAX_NUM_PEAKS_SPECTRUM
    ...
    if indexNumPeaks != 0:
        ...
        maxIndexBreathSpect    = computeMaxIndex(pVitalSigns_Breath_AbsSpectrum, breath_startFreq_Index, breath_endFreq_Index)
    ...
    numPeaks_heartSpectrum = find_Peaks(pVitalSigns_Heart_AbsSpectrum, pPeakIndex, pPeakValues,
                                        heart_startFreq_Index_1p6Hz, heart_endFreq_Index_4Hz)
    indexNumPeaks = numPeaks_heartSpectrum if (numPeaks_heartSpectrum < MAX_NUM_PEAKS_SPECTRUM) else MAX_NUM_PEAKS_SPECTRUM
    ...
    if (indexNumPeaks != 0):
        ...
        maxIndexHeartBeatSpect_4Hz = computeMaxIndex(pVitalSigns_Heart_AbsSpectrum, heart_startFreq_Index_1p6Hz, heart_endFreq_Index_4Hz)
        confidenceMetricHeartOut_4Hz = computeConfidenceMetric(pVitalSigns_Heart_AbsSpectrum,
                                                                    0,
                                                                    PHASE_FFT_SIZE/4,
                                                                    maxIndexHeartBeatSpect_4Hz,
    ...
    if (heartRateEst_FFT_4Hz < MAX_HEART_RATE_BPM):
        for indexTemp in range(1, numPeaks_heartSpectrum):
            if(abs(heartRateEst_FFT_4Hz - CONVERT_HZ_BPM * freqIncrement_Hz*pPeakSortOutIndex[indexTemp]) < HEART_HAMRONIC_THRESH_BPM):
                heartRateEst_FFT_4Hz = CONVERT_HZ_BPM *(freqIncrement_Hz)*pPeakSortOutIndex[indexTemp]
                break
            
    # Pick the Peaks in the Cardiac Spectrum
    numPeaks_heartSpectrum = find_Peaks(pVitalSigns_Heart_AbsSpectrum, pPeakIndex, pPeakValues, heart_startFreq_Index, heart_endFreq_Index)
    indexNumPeaks = numPeaks_heartSpectrum if (numPeaks_heartSpectrum < MAX_NUM_PEAKS_SPECTRUM) else MAX_NUM_PEAKS_SPECTRUM
    ...
    if (indexNumPeaks != 0):
        ...
        maxIndexHeartBeatSpect    = computeMaxIndex(pVitalSigns_Heart_AbsSpectrum, heart_startFreq_Index, heart_endFreq_Index)
    ...
    if(FLAG_MEDIAN_FILTER):
        ...
        heartRateEst_FFT = pBufferHeartRate[pPeakSortTempIndex[MEDIAN_WINDOW_LENGTH/2+1]]

        pPeakSortTempIndex = heapsort_index(pBufferHeartRate_4Hz, MEDIAN_WINDOW_LENGTH)
        heartRateEst_FFT_4Hz = pBufferHeartRate_4Hz[pPeakSortTempIndex[MEDIAN_WINDOW_LENGTH/2+1]]

        pPeakSortTempIndex = heapsort_index(pBufferBreathingRate, MEDIAN_WINDOW_LENGTH)
        breathingRateEst_FFT = pBufferBreathingRate[pPeakSortTempIndex[MEDIAN_WINDOW_LENGTH/2+1]]
    
    # Exponential Smoothing
    breathWfmOutPrev = breathWfmOutUpdated
    breathWfmOutUpdated = (alpha_breathing)*(outputFilterBreathOut*outputFilterBreathOut) + (1 - alpha_breathing)*breathWfmOutPrev # Exponential Smoothing
    sumEnergyBreathWfm = breathWfmOutUpdated*10000

    heartWfmOutPrev = heartWfmOutUpdated
    heartWfmOutUpdated = (alpha_heart)*(outputFilterHeartOut*outputFilterHeartOut) + (1 - alpha_heart)*heartWfmOutPrev  # Exponential Smoothing
